...
def filter_regulations(source_dir, dest_dir="processed_regulations", ranges=None, 
                      summarize=False, llm=None, chain_type="stuff", 
                      chunk_size=3500, chunk_overlap=300, short_threshold=1200,
                      overwrite_summaries=False, verbose=False):
    """
    ...
    regulation_dirs = [d for d in source_path.iterdir() 
            ...
            if copy_regulation(reg_dir, dest_reg_path):
                ...
                if summarize and llm is not None:
                    if summarize_regulation(
                        dest_reg_path, dest_path, llm, chain_type,
                        chunk_size, chunk_overlap, short_threshold,
                        overwrite_summaries, verbose
                    ):
                        summarized_count += 1
            else:
                skipped_count += 1
        else:
            print(f"- Skipping {reg_dir.name} (number {reg_number} not in target ranges)")
            skipped_count += 1
...
def build_llm(model: str = "gpt-4o", temperature: float = 0.1) -> Any:
    ...
    try:
        ...
        print("Error: langchain_openai is required for summarization. Install with: pip install langchain-openai")
        return None
...
def make_documents(text: str, reg_dir: str, chunk_size: int, chunk_overlap: int) -> List[Any]:
    ...
    try:
        ...
        print("Error: langchain and langchain_text_splitters are required for summarization.")
        return []
...
def summarize_text(
    ...
    try:
        ...
        raise RuntimeError(f"Summarization failed after {max_retries} attempts: {last_err}")
    except ImportError:
...
def summarize_regulation(
    ...
    source_files = [fname for fname in SUPPORTED_CONTENT_FILES if (reg_dir / fname).exists()]
    if not source_files:
        if verbose:
    ...
    if not overwrite and all((out_dir / f).exists() for f in source_files):
        if verbose:
            print(f"  Skip {reg_dir.name}: all summary files already exist (use --overwrite-summaries)")
        return True
    ...
    meta_data["summary_chars_each"] = {f: (out_dir / f).stat().st_size for f in written_files if (out_dir / f).exists()}
    meta_data["original_chars"] = len(raw_text)
    meta_dest.write_text(json.dumps(meta_data, ensure_ascii=False, indent=2), encoding="utf-8")
